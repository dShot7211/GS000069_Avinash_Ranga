...
import { SKUActionRenderer } from './SKUActionRenderer';
...
export const SKUTable: FunctionComponent<Props> = ({ gridTheme = 'ag-theme-quartz', isDarkMode }) => {
  const gridRef = useRef<AgGridReact>(null);
  const { skuRowData } = useSelector((store: RootState) => store.product);

  const [colDefs] = useState<ColDef[]>([
    { field: 'label', headerName: 'SKU', headerClass: 'header-sku', rowDrag: true },
    ...
    { field: 'actions', cellRenderer: SKUActionRenderer, minWidth: 194 }
  ]);
